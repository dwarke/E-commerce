...
import { createToken } from "../../adminPanel/auth/auth.middleware";
...
export const register = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const exist = await userRegisterModel.findOne({ email })
        if (exist) {
        ...
        const role = 'user'
        const user = new userRegisterModel({ name, email, password: hashPassword, role, phone, address, genderCategory });
        await user.save();
...
export const logout = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const logout = await userRegisterModel.findByIdAndUpdate(
            { _id: req.user?._id },
            { $pull: { token: token } }
        )
        if (!logout) {
